...
使用 google-generativeai 的 Automatic Function Calling。
"""
...
# ── Tool 定義（Gemini 會自動從 docstring 推斷 schema）────────────────────────
...
def memory_save(category: str, content: str) -> str:
    ...
    Returns:
        確認訊息
    """
    return save_memory(category, content)
...
def memory_recall(keyword: str = "") -> str:
    ...
    Returns:
        記憶列表文字
    """
    memories = recall_memories(limit=10)
    if not memories:
        return "目前沒有儲存任何記憶。"
    if keyword:
...
def _build_model() -> genai.GenerativeModel:
    return genai.GenerativeModel(
        ...
        tools=[web_search, memory_save, memory_recall, memory_delete, image_generate],
        system_instruction=SYSTEM_PROMPT,
    )


def _history_to_gemini(history: list[dict]) -> list[dict]:
    """將 Supabase 格式的對話記錄轉換成 Gemini 格式。"""
    result = []
    for turn in history:
...
def process_text(user_id: str, text: str) -> tuple[str, str | None]:
    ...
    # 加入記憶作為背景 context
    memories = recall_memories(limit=5)
    if memories:
        mem_text = "（背景記憶：" + "；".join(memories) + "）\n\n"
...
def process_pdf(user_id: str, pdf_bytes: bytes, filename: str = "文件") -> str:
    ...
    try:
        ...
        pages_text = []
        for page in reader.pages[:20]:  # 最多 20 頁
